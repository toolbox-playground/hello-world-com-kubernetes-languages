...
```bash
docker tag hello-world-java:latest localhost/hello-world-java:latest
```

4. Por último, verifique se a imagem foi criada corretamente:
```bash
docker images localhost/hello-world-java
```
...
kubectl create deployment my-deployment --image=localhost/hello-world-java:latest --namespace=my-namespace
...
kubectl describe deployment java-hello-world (opcional)
kubectl get service
...
2. **Acesso à aplicação**: A aplicação pode ser acessada através do endereço http://localhost:30000 após o deploy, dependendo da configuração do NodePort no serviço, e verá que sua aplicação java está Up & Running:
...
```bash
kind load docker-image localhost/hello-world-java --name <nome-do-cluster>
```
...
```bash
kubectl port-forward svc/java-hello-world-service 30000:8080
```
