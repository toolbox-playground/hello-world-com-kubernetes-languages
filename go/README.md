# Hello World Go com K8s
![Toolbox](../img/toolbox-playground.png)

Para usar o projeto Hello World Go com K8s, siga estes passos:

1. Certifique-se que você está dentro do diretório `hello-world-com-kubernetes-languages/go`.
...
```bash
docker tag hello-world-go:latest localhost/hello-world-go:latest
```

4. Por último, verifique se a imagem foi criada corretamente:
```bash
docker images localhost/hello-world-go
```
...
kubectl create deployment my-deployment --image=localhost/hello-world-go:latest --namespace=my-namespace
...
kubectl describe deployment go-hello-world (opcional)
kubectl get service
...
2. **Acesso à aplicação**: A aplicação pode ser acessada através do endereço http://localhost:30000 após o deploy, dependendo da configuração do NodePort no serviço, e verá que sua aplicação go está Up & Running:
...
```bash
kubectl port-forward svc/go-hello-world-service 30000:8080
```
